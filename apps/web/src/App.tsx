 ...
 */
export function App() {
  const [mode, setMode] = useState<"lesson" | "camera" | "samples">("lesson");
  ...
  return (
    <main>
      ...
      <nav className="mode-switch" aria-label="Demo mode"><button className={mode === "lesson" ? "active" : ""} aria-pressed={mode === "lesson"} onClick={() => setMode("lesson")}>Video lesson scaffold</button><button className={mode === "samples" ? "active" : ""} aria-pressed={mode === "samples"} onClick={() => setMode("samples")}>Dataset samples</button><button className={mode === "camera" ? "active" : ""} aria-pressed={mode === "camera"} onClick={() => setMode("camera")}>AR / HUD demo</button></nav>
      {mode === "samples" ? <DatasetSamplesDemo /> : mode === "camera" ? <CameraMode /> : error ? <div className="notice error" role="alert">{error}</div> : lesson ? <LessonMode lesson={lesson} /> : <div className="notice" role="status">Loading the demo lesson…</div>}
      <footer>Hackathon prototype · Sources shown with each view · Educational exploration only</footer>
    </main>
  );
}
