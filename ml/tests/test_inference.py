    ...
    def test_per_call_cutoff_changes_export_without_mutating_default_or_raw_predictions(self):
        ...
        for threshold, expected_count in ((0.9, 0), (0, 1), (1, 0), (None, 1)):
            with self.subTest(threshold=threshold):
                ...
                timings = {}
                profiled = adapter.predict_rgb_details(frame, rgb, threshold=threshold, timings=timings)
                self.assertEqual(profiled[0], result)
                np.testing.assert_array_equal(profiled[1], raw)
                self.assertTrue(timings, 'profiling and a per-call cutoff must work together')
        with tempfile.TemporaryDirectory() as directory:
